...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y[torch.arange(len(x)), x_tensor] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu = x.sum(dim=0) / M
    x_centered = x - mu
    sigma = ((x_centered ** 2).sum(dim=0) / M) ** 0.5
    y = x_centered / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    max_len = ls.max().item()
    padded = torch.zeros(len(xs), max_len, dtype=xs[0].dtype, device=xs[0].device)
    for i, (tensor, length) in enumerate(zip(xs, ls)):
        padded[i, :length] = tensor[:length]
    
    # Create mask and compute mean
    mask = torch.arange(max_len, device=ls.device).expand(len(xs), max_len) < ls.unsqueeze(1)
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    uniques, inverse_indices = torch.unique(x, return_inverse=True)
    indices = torch.zeros_like(uniques)
    for i in range(len(uniques)):
        indices[i] = (inverse_indices == i).nonzero()[0]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
