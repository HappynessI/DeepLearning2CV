...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # Conv layer: input (C, H, W), output (num_filters, H, W)
        ...
        # After pooling: (num_filters, H/2, W/2)
        # Linear layer input size
        linear_input_size = num_filters * (H // 2) * (W // 2)
        
        # Hidden linear layer
        self.params['W2'] = weight_scale * torch.randn(linear_input_size, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dout1, cache1)
    ...
    def __init__(self,
        ...
        for i, num_filter in enumerate(num_filters):
            if weight_scale == 'kaiming':
                self.params[f'W{i+1}'] = kaiming_initializer(prev_filters, num_filter, K=3, relu=True, device=device, dtype=dtype)
            else:
                self.params[f'W{i+1}'] = weight_scale * torch.randn(num_filter, prev_filters, 3, 3, dtype=dtype, device=device)
        ...
        linear_input_size = num_filters[-1] * H * W
        if weight_scale == 'kaiming':
            self.params[f'W{len(num_filters)+1}'] = kaiming_initializer(linear_input_size, num_classes, K=None, relu=False, device=device, dtype=dtype)
        else:
            self.params[f'W{len(num_filters)+1}'] = weight_scale * torch.randn(linear_input_size, num_classes, dtype=dtype, device=device)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    model = DeepConvNet(num_filters=[16, 32], max_pools=[0, 1], 
                        weight_scale='kaiming', reg=1e-3, 
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        if cache is None:
            ...
            dmean = (dx_normalized * -1 / torch.sqrt(sample_var + eps)).sum(dim=0) + dvar * (-2 / N) * (x - sample_mean).sum(dim=0)
    ...
    def backward_alt(dout, cache):
        ...
        if cache is None:
            ...
            dgamma = (dout * x_normalized).sum(dim=0)
            dx_normalized = dout * gamma
            dx = (1. / N) * (1. / torch.sqrt(sample_var + eps)) * (N * dx_normalized - dx_normalized.sum(dim=0) - x_normalized * (dx_normalized * x_normalized).sum(dim=0))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
